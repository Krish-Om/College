...
class Game:
    ...
    def _is_potential_move_colliding(self, next_head_x, next_head_y):
        ...
        return False  # No collision    def _calculate_distance(self, x1, y1, x2, y2):
        """
    ...
    def random_agent(self):
        """
        This is a simple random 'AI' agent that makes the snake move randomly.
        It randomly picks one of four directions (left, right, up, down).
        """
        day_number = random.randint(1, 4)  # Randomly select an integer from 1 to 4.
        if day_number == 1:
    ...
    def agent_P(self):
        """
        A simple pathfinding agent that optimizes score while preventing collisions.
        Uses a safety-first approach with Manhattan distance heuristics to navigate efficiently.
        """
        # Get current head position and apple position
        head_x, head_y = self.snake.x[0], self.snake.y[0]
        apple_x, apple_y = self.apple.x, self.apple.y

        # Define possible directions to evaluate
        directions = ["left", "right", "up", "down"]
        valid_moves = []

        # Calculate scores for each possible move
        for direction in directions:
            # Skip directions that would cause immediate U-turns
            if (
                (direction == "left" and self.snake.direction == "right")
                or (direction == "right" and self.snake.direction == "left")
                or (direction == "up" and self.snake.direction == "down")
                or (direction == "down" and self.snake.direction == "up")
            ):
                continue

            # Get the potential next head position
            next_head_x, next_head_y = self._get_potential_head(direction)

            # Skip if move would result in collision
            if self._is_potential_move_colliding(next_head_x, next_head_y):
                continue

            # Calculate Manhattan distance (grid-based distance) to apple after this move
            # This is more appropriate than Euclidean for grid movement
            manhattan_dist = abs(next_head_x - apple_x) + abs(next_head_y - apple_y)

            # Check safety of this move (space available after making this move)
            safety_score = self._evaluate_safety(next_head_x, next_head_y)

            # Calculate a combined score - lower is better
            # We prioritize safety slightly higher than distance to apple
            move_score = manhattan_dist - (safety_score * 2)

            # Store valid move with its score
            valid_moves.append((direction, move_score))

        # If no valid moves found, try to find any non-colliding move as a fallback
        if not valid_moves:
            for direction in directions:
                next_head_x, next_head_y = self._get_potential_head(direction)

                # Even if it's a U-turn, consider it if it's the only non-colliding option
            ...
            # If we get here, all moves lead to collision - just continue in current direction
            return

        # Sort moves by score (lowest score = best move)
