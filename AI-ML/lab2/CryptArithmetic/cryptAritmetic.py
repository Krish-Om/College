def solve_cryptarithmetic(puzzle):
    ...
    # letter_set:set = set(c for c in expression+result if "A" <= c <="Z")


    # Extract unique letters 
    # there should not be more that 9 unique letters
    letters = sorted(list(set(c for c in expression + result if "A" <= c <= "Z")))
    if len(letters) > 10:
        print(
            "Error: More than 10 unique letters. Cryptarithmetic puzzles typically use 0-9."
        )
        return None
        ...
        try:
            ...
            for char in expression:
                if char in "+-*/":
                    ...
                    if current_index < len(words) - 1 and words[
                        current_index
                    ].startswith(char):
            ...
            ):  # check to make sure words isn't exhausted
                eval_expression += str(num_words[current_index])
