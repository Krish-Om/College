...
def water_jug_bfs(jug1_capacity=0, jug2_capacity=0, target=2):
    visited = set()
    queue = deque()
...
def water_jug_dfs(jug1_capacity=0, jug2_capacity=0, target=2):
    visited = set()
    stack = []
    parent = {}
    action_taken = {}
    ...
    while stack:
        ...
        if jug1 == target or jug2 == target:
            ...
            print(f"✅ Target {target} can be reached with state ({jug1}, {jug2})")
            print("=" * 60)
            ...
            for step, (state, action) in enumerate(zip(path, actions)):
                ...
                if step < len(path) - 1:
                    print("    ↓")
            print(f"\n🏆 Success! Reached target {target} in {len(path) - 1} steps!")
            return True
        ...
        possible_states = [
            ...
            (
                ...
                f"5: Pour jug1 → jug2 (transfer {min(jug1, jug2_capacity - jug2)} units)",
            ),
            (
                (
                    jug1 + min(jug2, jug1_capacity - jug1),
                ...
                f"6: Pour jug2 → jug1 (transfer {min(jug2, jug1_capacity - jug1)} units)",
            ),
        ]
